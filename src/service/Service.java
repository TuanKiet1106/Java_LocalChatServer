...
public class Service {
    ...
    public void startServer() {
        ...
        server.addEventListener("send_request_to_user", ModelSendMessage.class, new DataListener<ModelSendMessage>() {
            @Override
            public void onData(SocketIOClient sioc, ModelSendMessage t, AckRequest ar) throws Exception {
                sendRequestToClient(t);
            }
        });
    ...
    private void sendRequestToClient(ModelSendMessage data) {
        for (ModelClient c : listClient) {
            if (c.getUser().getUserID() == data.getToUserID()) {
                c.getClient().sendEvent("receive_rq", new ModelReceiveMessage(data.getMessageType(),data.getFromUserID(), data.getText(), null));
                break;
            }
        }
    }
